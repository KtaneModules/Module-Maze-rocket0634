...
public partial class CommunityFeaturesDownloader : EditorWindow
{
    ...
    {
        Debug.Log("Test");
        if (DateTime.Now.TotalSeconds() - realTime > 1)
        {
            realTime = DateTime.Now.TotalSeconds();
            Debug.Log("Disabling OnEditorUpdate");
            Debug.Log("Test2");
    ...
    private void OnGUI()
    {
        Debug.LogFormat("OnGUI called {0}", num++);
        Debug.Log(realTime);
        ...
        try
        {
            ...
            {
                Debug.Log("Enabling OnEditorUpdate");
                EditorApplication.update += OnEditorUpdate;
